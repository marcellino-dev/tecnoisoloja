...
const CATEGORIES = [
  { label: 'Termômetros',   slug: 'termometros' },
  { label: 'Manômetros',    slug: 'manometros' },
            ...
            <div className="flex items-center gap-3 flex-none">
              ...
              {session ? (
                <div className="relative" ref={userRef}>
                  ...
                  >
                    {session.user?.image ? (
                      <Image src={session.user.image} alt="" width={28} height={28} className="rounded-full ring-2 ring-white/30" />
                    ) : (
                    ...
                    <div className="hidden sm:block text-left leading-tight">
                      <p className="text-[10px] text-white/70">Olá, {session.user?.name?.split(' ')[0]}</p>
                      <p className="text-xs font-700 flex items-center gap-0.5">
                        Minha conta <ChevronDown className={cn('w-3 h-3 transition-transform', userMenuOpen && 'rotate-180')} />
                      </p>
                    </div>
                  </button>
                  ...
                  {userMenuOpen && (
                    <div className="absolute right-0 top-full mt-3 w-52 bg-white rounded-xl shadow-2xl border border-gray-100 py-1 animate-slide-down">
                      <div className="px-4 py-3 border-b border-gray-100">
                        ...
                        {isAdmin && (
                          <span className="text-[10px] bg-brand-100 text-brand-700 px-2 py-0.5 rounded-full font-600 mt-1 inline-block">Admin</span>
                        )}
                      </div>
        ...
        <div className="container-custom">
          <div className="flex items-center gap-6 h-10 text-white text-xs font-500">
            ...
            <div className="relative" ref={catRef}>
              ...
              >
                Categorias <ChevronDown className={cn('w-3.5 h-3.5 transition-transform', catMenuOpen && 'rotate-180')} />
              </button>
        ...
        <div className="sm:hidden bg-dark-900/95 backdrop-blur-md border-t border-white/10 py-3 animate-slide-down">
          <div className="container-custom flex flex-col gap-1">
            {[
              ...
              { href: '/cart',     label: `Carrinho (${count})` },
